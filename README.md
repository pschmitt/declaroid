...
Install these 3 app(s) on 10.5.0.110:43411? [y/N/m] y
...
Answering `m`/manual instead of `y` at any install/uninstall/`--enforce` prompt
...
| `-y, --yes, --noconfirm, --no-confirm` | apply, apply --enforce, uninstall, clear-cache | Skip the confirmation prompt. Apply's app/module install/uninstall/`--enforce` prompts also accept `m`/manual (`[y/N/m]`), which opens an fzf multi-select to narrow the batch down to specific apps/modules instead of all-or-nothing (requires `fzf`) |
...
Install these 1 module(s) on 10.5.0.159:37819? [y/N/m] y
